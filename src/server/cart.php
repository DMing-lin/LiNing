...
if($type == "add")
{
  ...
  {
    /* 往数据库表中新增一条数据 */
    $sql = "INSERT INTO `cart` (`good_id`, `num`) VALUES ($good_id, 1)";
  }else{
    /* 更新数据 */
  ...
  // $sql = "SELECT cart.*,list.name,list.img,list.price FROM cart , list WHERE cart.good_id = list.uid";
  $sql = "SELECT cart.num,cart.good_id,list.name,list.img,list.price FROM cart , list WHERE cart.good_id = list.uid";
